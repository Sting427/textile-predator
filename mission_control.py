...
st.set_page_config(page_title="TEXTILE PREDATOR // CLASSIFIED", layout="wide", page_icon="💀")
...
def check_password():
    ...
    if "password_correct" not in st.session_state:
        ...
        st.text_input(
            "Enter Clearance Code", type="password", on_change=password_entered, key="password"
        )
        return False
...
if check_password():
    ...
    # --- 🖥️ DASHBOARD UI ---
    st.title("💀 TEXTILE PREDATOR // COMMAND v7.0")

    with st.spinner("Authenticating & Syncing with Global Markets..."):
        df = load_market_data()
    ...
    with tab4:
        ...
        with colA:
            ...
            knitting_cost = st.number_input("Knitting Cost ($/kg)", value=0.60)
            overhead_cost = st.number_input("Overhead Cost ($/kg)", value=0.15)
        with colB:
